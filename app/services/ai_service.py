        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "slot_code": {
                        "type": "string",
                        "description": "时段代码：T1=上午第一节，T2=上午第二节，T3=下午第一节，T4=下午第二节。仅当用户明确指定第几节时才传入；用户只说'上午'或'下午'时不传此参数，由后续回复过滤。",
                        "enum": ["T1", "T2", "T3", "T4"]
                    },
                    "classroom": {
                        "type": "string",
...
- 调用 query_classrooms 工具时，必须严格按用户描述提取参数，不要遗漏：
  ...
  - 时段：工具只支持单个时段代码，因此：
      * 用户说"上午第一节"→ slot_code="T1"，"上午第二节"→"T2"
      * 用户说"下午第一节"→ slot_code="T3"，"下午第二节"→"T4"
      * 用户说"上午"（不指定第几节）→ 不传 slot_code，让工具返回该日所有时段，你在回复中只展示上午（T1、T2）的结果
      * 用户说"下午"（不指定第几节）→ 不传 slot_code，回复中只展示下午（T3、T4）的结果
      * 用户没提时段 → 不传此参数
