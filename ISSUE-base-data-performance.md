# 【待验证】基础数据页面切换 Tab 卡顿问题

## 问题描述

**环境**: Windows, 排考系统前端 (Vue 3 + Vite + Element Plus)

**现象**: 另一台电脑上切换到「基础数据」页面后出现明显卡顿。ElTag type="" 警告已修复，但卡顿仍然存在，怀疑是某个地方在持续发请求或反复渲染。

> ⚠️ **后续发现**: 在本机停掉后端服务后测试，页面不再卡顿。可能原因：
> 1. 另一台电脑性能较差
> 2. 网络环境导致请求超时反复切换 mock/真实模式
> 3. 其他未知因素
...
| `CrudTab.vue` | 搜索防抖 300ms + Mock模式请求拦截 | 防止频繁请求和模式反复切换 ✅ |

---

## 待排查方向（已部分修复）

> 以下方向已在 `cdb2e73` 提交中修复：
> - ✅ 搜索框防抖
> - ✅ Mock 模式切换逻辑问题

### 1. 请求循环（最可疑）~~检查 `useCrud.js` 中的 `fetchData()` 是否在某处被重复触发。可能的触发路径：~~
- `watch` 监听器触发
- `onMounted` 重复调用
- `computed` 间接触发
